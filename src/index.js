...
module.exports = function (botKey, botSecret, publicId) {
	const botAuth = retry(function (cb) {
			request({
				method: 'POST',
				...
				if (res.statusCode !== 200) {
					console.log('Could not authenticate bot: Did not receive 200 http status, instead was: ', res.statusCode, ' body: ', body);
	...
	const retrieveUserSlug = retry(function (cb) {
			request({
	...
	getParser(roomId, function (err, parseMessage) {
		...
		botAuth(function (err, token) {
			...
			retrieveUserSlug(function (err, channelSlug) {
				if (err) {
					console.log(err);
					return;
				}
